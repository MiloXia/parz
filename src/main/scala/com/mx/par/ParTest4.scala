...
object ParTest4 extends App {
  ...
  object FetchEffect extends (Request ~> Id) { //F[A] is Requests[Responses]
    ...
    private def translate[A](req: Request[A]): Id[A] = {
      Service.deal(req)
    }
  ...
  //service
  trait Service {
    def deal[A]: PartialFunction[Request[A], A]
  }
  object Service extends Service {
    def deal[A] = List(TestService, GotoWorkService).map(_.deal[A]).reduceLeft(_ orElse _)
  }
  ...
  object GotoWorkService extends Service {
    def deal[A] = {
      case GetUp =>
        println("get up")
      case Wash =>
